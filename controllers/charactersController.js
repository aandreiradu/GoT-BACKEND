...
const getAllCharacters = async (req, res) => {
  ...
  if(!characters){
    return res.status(204).json({ message: "No characters found" });
  }

  return res.status(200).json(characters);
};

const checkCharacterExisting = async (character) => {
  if (!character) {
    return {
      response: true,
      message: "Invalid payload, no character provided to check!",
    };
  }
  const { fullName } = character || null;
  if (!fullName) {
    return {
      response: true,
      message: "Invalid payload, character provided without full name!",
    };
  }

  const existingCheck = await Character.findOne({ fullName }).exec();
  console.log("existingCheck", existingCheck);

  if (existingCheck) {
    return {
      response: true,
      message: "Character existing!",
    };
  } else {
    return false;
  }
...
const insertCharacter = async (req, res) => {
  ...
  if (Array.isArray(character)) {
    return res
      .status(400)
      .json({ message: "You cant insert multiple characters" });
  }

  const existingResponse = await checkCharacterExisting(character);
  console.log("existingResponse", existingResponse);

  // check existing character;
  if (existingResponse) {
    return res.status(409).json({ existingResponse });
  }

  try {
    const insertResponse = await Character.create(character);
...
const getCharacterByID = async (req,res) => {
  ...
  if(!id) {
    return res.status(400).json({message: 'Invalid payload. Provide character id'});
  }

  const getCharFromDB = await Character.findOne({_id : id}).exec();

  if(!getCharFromDB) {
    return res.status(404).json({message : `No character found for ID ${id}`});
  }

  return res.status(200).json(getCharFromDB);
}
